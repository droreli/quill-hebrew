...
@Test func LMStudioConfigurationOnlyAcceptsLiteralLoopbackRoots() throws {
  ...
  #expect(
    try LMStudioConfiguration(
      endpoint: "http://127.0.0.1:1234", modelID: "local/custom", isEnabled: true
...
@Test func chunkingPreservesSegmentBoundariesAndRefusesOversizedSegment() throws {
  ...
  let source = try SessionTranscript(
    engine: "fixture", model: "fixture", createdAt: "2026-09-01T00:00:00Z", speakerLabels: true,
    timestamps: true,
    segments: [
      .init(id: "s000001", speaker: "me", startMS: 0, endMS: 100, text: "first bounded segment"),
      .init(
        id: "s000002", speaker: "them", startMS: 100, endMS: 200, text: "second bounded segment"),
    ]
  )
...
@Test func malformedUnknownAndUnavailableLocalResponsesFailSafely() async throws {
  ...
  let malformedEngine = LMStudioSummarizationEngine(
    configuration: try configuration(port: malformedPort), session: fakeSession())
  ...
  FakeLMStudioURLProtocol.configure(
    port: unknownPort,
    responses: [.success(openAIResponse(payload: payload(evidenceID: "s999999")))])
  let unknownEngine = LMStudioSummarizationEngine(
    configuration: try configuration(port: unknownPort), session: fakeSession())
  ...
  FakeLMStudioURLProtocol.configure(
    port: unavailablePort, responses: [.http(status: 503, data: Data())])
  let unavailableEngine = LMStudioSummarizationEngine(
    configuration: try configuration(port: unavailablePort), session: fakeSession())
  await #expect(throws: LMStudioError.self) {
    try await unavailableEngine.summarize(
      transcript: transcript(), rawNotes: notes(), input: input())
  }
}
...
@Test func disabledProviderAndResponseCapAvoidInference() async throws {
  ...
  FakeLMStudioURLProtocol.configure(
    port: cappedPort, responses: [.success(Data(repeating: 65, count: 256))])
  let cappedConfiguration = try LMStudioConfiguration(
    endpoint: "http://127.0.0.1:\(cappedPort)", modelID: "local/test-model", isEnabled: true,
    maximumResponseBytes: 32)
  let capped = LMStudioSummarizationEngine(
    configuration: cappedConfiguration, session: fakeSession())
  await #expect(throws: LMStudioError.self) {
...
@Test func emptyTranscriptIsRejectedBeforeAnyModelRequest() async throws {
  ...
  let engine = LMStudioSummarizationEngine(
    configuration: try configuration(port: port), session: fakeSession())
  ...
  await #expect(throws: LMStudioError.self) {
    try await engine.summarize(
      transcript: empty, rawNotes: notes(),
      input: .init(transcriptSHA256: "fixture", transcriptSegmentCount: 0, rawNotesRevision: 4))
  }
...
@Test func knownEvidenceIDDoesNotUpgradeInventedModelClaimToVerified() throws {
  ...
  let modelPayload = payload(
    evidenceID: "s000001", decisionText: "The board approved a budget of $10M.")
  let brief = try decoder.makeMeetingBrief(
    payload: modelPayload, transcript: transcript(), input: input(),
    configuration: try configuration(port: 15107), createdAt: "2026-09-01T00:00:00Z",
    id: "trust-test"
  )
...
@Test func decoderRejectsUncitedStructuredModelItems() throws {
  ...
  #expect(throws: LMStudioError.self) {
    try decoder.makeMeetingBrief(
      payload: invalid, transcript: transcript(), input: input(),
      configuration: try configuration(port: 15108))
  }
}

@Test func longMeetingsReduceInBoundedHierarchicalBatchesAndNamespaceModelIDs() async throws {
  let port = 15109
  let extractionResponses = (1...4).map { index in
    FakeLMStudioURLProtocol.Reply.success(
      openAIResponse(payload: payload(evidenceID: String(format: "s%06d", index))))
  }
  let firstLayer = [
    FakeLMStudioURLProtocol.Reply.success(openAIResponse(payload: payload(evidenceID: "s000001"))),
    FakeLMStudioURLProtocol.Reply.success(openAIResponse(payload: payload(evidenceID: "s000003"))),
  ]
  let final = BriefResponseDecoder.ModelBriefPayload(
    language: "english", overview: "Final reduction.",
    topics: [.init(id: "item-1", text: "A topic", evidenceSegmentIDs: ["s000001"])],
    decisions: [.init(id: "item-1", text: "A decision", evidenceSegmentIDs: ["s000004"])],
    actionItems: [], openQuestions: [], warnings: []
  )
  FakeLMStudioURLProtocol.configure(
    port: port,
    responses: extractionResponses + firstLayer + [.success(openAIResponse(payload: final))])
  let engine = LMStudioSummarizationEngine(
    configuration: try LMStudioConfiguration(
      endpoint: "http://127.0.0.1:\(port)", modelID: "local/test-model", isEnabled: true,
      chunkTokenBudget: 40),
    session: fakeSession()
  )

  let manySegments = try SessionTranscript(
    engine: "fixture", model: "fixture", createdAt: "2026-09-01T00:00:00Z", speakerLabels: true,
    timestamps: true,
    segments: (1...4).map { index in
      .init(
        id: String(format: "s%06d", index), speaker: "me", startMS: index * 100,
        endMS: index * 100 + 50, text: "A bounded discussion point \(index).")
    }
  )
  let brief = try await engine.summarize(
    transcript: manySegments,
    rawNotes: try notes(),
    input: .init(transcriptSHA256: "fixture", transcriptSegmentCount: 4, rawNotesRevision: 4)
  )

  #expect(FakeLMStudioURLProtocol.requests(port: port).count == 7)
  #expect(try #require(brief.topics.first).id == "final-topic-1")
  #expect(try #require(brief.decisions.first).id == "final-decision-1")
}

private func configuration(port: Int) throws -> LMStudioConfiguration {
  try LMStudioConfiguration(
    endpoint: "http://127.0.0.1:\(port)", modelID: "local/test-model", isEnabled: true,
    chunkTokenBudget: 128)
}
...
private func transcript() throws -> SessionTranscript {
  try SessionTranscript(
    ...
    segments: [
      ...
      .init(
        id: "s000002", speaker: "them", startMS: 100, endMS: 200,
        text: "Dana will send the proposal Friday."),
    ]
  )
}
...
private func notes() throws -> RawMeetingNotes {
  try RawMeetingNotes(
    sessionID: "fixture", revision: 4, template: "general", updatedAt: "2026-09-01T00:00:00Z",
    notes: [])
}
...
private func payload(evidenceID: String, decisionText: String = "Run a pilot")
...
private final class FakeLMStudioURLProtocol: URLProtocol, @unchecked Sendable {
  ...
  override func startLoading() {
    ...
    let reply =
      Self.repliesByPort[port, default: []].isEmpty ? nil : Self.repliesByPort[port]!.removeFirst()
    Self.lock.unlock()
